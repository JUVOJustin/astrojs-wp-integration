---
title: Actions
description: Configure Astro server actions for WordPress content, terms, users, and abilities with shared auth and consistent error handling.
---
...
Every action factory works directly with a `WordPressClient` or a request-aware client resolver such as `wordPressAuthBridge.getClient`. Under the hood, each action delegates to `fluent-wp-client` and maps failures to `ActionError`.

<CardGrid>
  <LinkCard title="Post actions" href="./posts.mdx" description="Create, update, and delete posts, pages, and custom post types." />
  <LinkCard title="Term actions" href="./terms.mdx" description="Manage categories, tags, and custom taxonomies." />
  <LinkCard title="User actions" href="./users.mdx" description="Create, update, and delete WordPress users." />
  <LinkCard title="Ability actions" href="./abilities.mdx" description="Execute WordPress abilities through Astro actions." />
</CardGrid>

## Integration testing

Action integration tests run through a real Astro fixture app and call `/_actions/*` endpoints over HTTP.
This validates routing, input parsing, handler execution, and serialized responses as one end-to-end flow.

Use `npm run test:integration` for action/loaders/auth integration checks through the real Astro fixture app.

---

## Authentication

<Aside type="note" title="Prefer one shared client resolver">
Use `wordPressAuthBridge.getClient` as the shared action input when auth depends on the current Astro request.
</Aside>

All write operations use one `WordPressClient` or one request-aware client resolver.

### Client-first (recommended)
...
export const server = {
  ...
  createCategory: createCreateTermAction(wordPressAuthBridge.getClient, { resource: 'categories' }),
  createUser: createCreateUserAction(wordPressAuthBridge.getClient),
  getAbility: createGetAbilityAction(wordPressAuthBridge.getClient),
};
```

### Static client (app password)
...
### Static client with request-aware auth headers
...
Use the same client in all factories when you want one shared static auth setup:
...
export const server = {
  ...
  getAbility: createGetAbilityAction(wp),
  runAbility: createRunAbilityAction(wp),
  deleteAbility: createDeleteAbilityAction(wp),
};
```

<CardGrid>
  <LinkCard title="User actions" href="./users.mdx" description="Delete flow requirements, including mandatory reassignment." />
  <LinkCard title="Term actions" href="./terms.mdx" description="Category, tag, and custom taxonomy resource behavior." />
</CardGrid>
...
Use `mapResponse` on create/update post actions when the successful WordPress response needs site-specific normalization before Astro receives it, such as replacing ACF choice values with labels from discovery metadata.

See [Mapping WordPress Values](../mapping.mdx) for a reusable callback pattern that works with both loaders and actions.
...
- **`create*Action(client)`** — the Astro `ActionClient` factory for use in `src/actions/index.ts`.
- **`execute*(client, input, options?)`** — the raw async helper, useful for testing or building custom actions that target a different REST resource (e.g. a custom post type).
...
const product = await executeCreatePost(
  wp,
  { title: 'My Product', status: 'publish' },
  { resource: 'products' },
);
```

If you use JWT, build the client with `auth: { token }` instead.

When `resource` is omitted it defaults to `'posts'` for content actions and `'categories'` for term actions.

<CardGrid>
  <LinkCard title="Post actions: resources" href="./posts.mdx#pages-and-custom-post-types" description="How `resource` targets pages and custom post type endpoints." />
  <LinkCard title="Term actions" href="./terms.mdx" description="Resource usage for built-in and custom taxonomies." />
  <LinkCard title="Ability actions" href="./abilities.mdx" description="Action wrappers for the WordPress abilities API." />
</CardGrid>
