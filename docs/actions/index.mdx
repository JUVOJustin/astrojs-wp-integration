...
Every action is a factory function that binds a WordPress auth config (static credentials or request resolver) and returns a reusable Astro `ActionClient`. Under the hood, each action calls an `execute*` function that performs the HTTP request and maps errors to `ActionError`.
...
const wpConfig = {
  ...
  authHeaders: ({ method, url, body }) => {
    ...
    const signature = signWordPressRequest({ method, url: url.toString(), body });
