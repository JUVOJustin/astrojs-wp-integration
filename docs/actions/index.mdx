...
Every action factory can now work directly with a `WordPressClient` or a request-aware client resolver such as `wordPressAuthBridge.getClient`. Under the hood, each action delegates to `fluent-wp-client` and maps failures to `ActionError`.
...
All write operations now use one `WordPressClient` or one request-aware client resolver.
