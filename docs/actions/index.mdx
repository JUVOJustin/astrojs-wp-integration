...
Every action is a factory function that binds a WordPress auth config (static credentials or request resolver) and returns a reusable Astro `ActionClient`. Under the hood, each action calls an `execute*` function that uses the same low-level `WordPressClient` request transport and maps failures to `ActionError`.
...
When `resource` is omitted it defaults to `'posts'`. See [Post actions — Pages and custom post types](./posts.mdx#pages-and-custom-post-types) for the full pattern.
