...
describe('Actions: User Data Isolation', () => {
  ...
  it('returns alice profile when authenticated as alice', async () => {
    const profile = await callAction<{
      ...
      slug: string;
      name: string;
    }>('getCurrentUserProfile', {}, { authHeader: aliceAuth });
  ...
  it('returns bob profile when authenticated as bob', async () => {
    const profile = await callAction<{
      ...
      slug: string;
      name: string;
    }>('getCurrentUserProfile', {}, { authHeader: bobAuth });
  ...
  it('does not leak alice profile to bob across sequential requests', async () => {
    const aliceProfile = await callAction<{
      id: number;
      slug: string;
      name: string;
    }>('getCurrentUserProfile', {}, { authHeader: aliceAuth });

    const bobProfile = await callAction<{
      id: number;
      slug: string;
      name: string;
    }>('getCurrentUserProfile', {}, { authHeader: bobAuth });
  ...
  it('does not leak alice profile to bob when bob reuses the same action payload', async () => {
    ...
    const aliceProfile = await callAction<{
      ...
      slug: string;
      name: string;
    }>('getCurrentUserProfile', sharedInput, { authHeader: aliceAuth });

    const bobProfile = await callAction<{
      id: number;
      slug: string;
      name: string;
    }>('getCurrentUserProfile', sharedInput, { authHeader: bobAuth });
  ...
  it('rejects unauthenticated requests for user profile', async () => {
    await expect(callAction('getCurrentUserProfile', {})).rejects.toThrow(
      ActionError,
    );
  });
