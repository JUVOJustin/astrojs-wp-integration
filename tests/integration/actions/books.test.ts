...
describe('Actions: Books', () => {
  ...
  afterAll(async () => {
    for (const id of createdIds) {
      await callAction('deleteBook', { id, force: true }, { authHeader: basicAuth }).catch(() => undefined);
    }
  });

  it('routes create action to custom post type resource endpoint', async () => {
    const created = await callAction<{ id: number; type: string }>('createBook', {
  ...
  it('supports custom schema extension for CPT-specific fields', async () => {
    const created = await callAction<{ id: number; type: string }>('createBookCustomSchema', {
  ...
  it('supports response schema override for CPT action responses', async () => {
    const created = await callAction<{ id: number; type: 'book'; status: string }>('createBookResponseOverride', {
  ...
  it('updates CPT content through configured books resource', async () => {
    const created = await callAction<{ id: number }>('createBook', {
      title: 'Books behavior: update base',
      status: 'draft',
    }, { authHeader: basicAuth });
    createdIds.push(created.id);

    const updated = await callAction<{ type: string; title: { rendered: string } }>('updateBook', {
      id: created.id,
  ...
  it('maps CPT write auth failures to ActionError', async () => {
    await expect(
      callAction('createBook', { title: 'Books behavior: unauthorized', status: 'draft' }),
