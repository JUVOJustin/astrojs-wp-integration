...
- The package is aligned to `fluent-wp-client` `^3.0.0-rc.7`; loader payloads for post/page resources must remain plain serializable objects (no legacy helper methods on `entry.data`).
...
| Media | 1 | `wpimage-test-media`; generated PNG attachment used by WPImage integration tests |
...
WPImage build coverage uses the seeded `wpimage-test-media` attachment to
verify default Astro `<Picture>` output emits local image assets and modern
formats. It also verifies `optimize="never"` renders WordPress `src`/`srcset`.
